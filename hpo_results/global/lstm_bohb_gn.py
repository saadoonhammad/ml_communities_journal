# ============================================================================
# LSTM AUTOENCODER - BAYESIAN HYPERBAND OPTIMIZATION (BOHB) - FUSED MODEL
# ============================================================================
"""
Bayesian Hyperband Optimization for LSTM Autoencoder using Optuna.
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'../../Logs/global_lstm_bohb_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
...
OUTPUT_PREFIX = f'{CLUSTER}_global_lstm_bohb_results'
...
def prepare_multisensor_data(df: pd.DataFrame, sequence_length: int) -> Tuple[np.ndarray, List[str]]:
    """Prepare multi-sensor data for LSTM autoencoder."""
...
class FusedLSTMAutoencoder:
    """LSTM Autoencoder for multi-sensor time series."""
    ...
    def objective(self, trial: optuna.Trial) -> float:
        ...
        try:
            for fold_idx, (df_train_fold, df_val_fold) in enumerate(self.data_splits):
                ...
                # Build model
                autoencoder = FusedLSTMAutoencoder(
...
def save_bohb_results(results: Dict[str, Any], output_dir: str = '../../HPO Results/Network_Global/global_lstm_bohb_results'):
    ...
    json_results = {
        ...
        'model_architecture': 'LSTM_Autoencoder',   # added for clarity
        'cluster': CLUSTER,

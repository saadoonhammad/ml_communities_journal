# ============================================================================
# LSTM AUTOENCODER - BAYESIAN HYPERPARAMETER OPTIMIZATION WITH FEATURE ENGINEERING
# ============================================================================
"""
Hyperparameter optimization for single-layer LSTM Autoencoder using Bayesian Optimization
...
Architecture: Single-layer LSTM with tanh activation
...
Objective: Minimize validation reconstruction error (MSE)d
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'lstm_fe_hpo_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
...
logger.info("="*80)
logger.info("LSTM AUTOENCODER - BAYESIAN HPO WITH FEATURE ENGINEERING")
logger.info("="*80)
...
class LSTMAutoencoder:
    ...
    Uses standard unidirectional LSTM (not bidirectional).
    
    Architecture (1 layer):
        Input → LSTM → Dense(encoding) → RepeatVector 
        → LSTM → TimeDistributed(Dense) → Output
    
    Architecture (2 layers):
        Input → LSTM → LSTM → Dense(encoding) 
        → RepeatVector → LSTM → LSTM 
    ...
    def __init__(self, sequence_length, n_features=1,
                 ...
                 use_lr_warmup=True):
        """
        Initialize LSTM Autoencoder (single-layer configuration).
        ...
        Args:
            ...
            lstm_units_2: not used for single layer (kept for compatibility)
    ...
    def build_model(self):
        """Build the LSTM Autoencoder (unidirectional)."""
        inp = Input(shape=(self.sequence_length, self.n_features))

        # Encoder (Standard LSTM - no Bidirectional wrapper)
        ...
        # Decoder (Standard LSTM - no Bidirectional wrapper)
...
class HyperparameterOptimizer:
    """
    Bayesian Hyperparameter Optimization for Single-Layer LSTM Autoencoder with Feature Engineering.
    ...
    def define_search_space(self):
        """
        Define hyperparameter search space for single-layer LSTM.
        
        Note: LSTM units range doubled compared to BiLSTM since no concat mode
        ...
        return [
            ...
            Integer(8, 128, name='encoding_dim'),
    ...
    def _get_checkpoint_filename(self):
        ...
        return (f"lstm_fe_checkpoint_s{self.sequence_length}_"
    ...
    def objective_function(self, lstm_units, encoding_dim, learning_rate, 
        ...
        3. Trains single-layer LSTM model with given hyperparameters
        ...
        try:
            for fold, (train_data_df, val_data_df) in enumerate(self.data_splits):
                ...
                # Build and train model (single-layer LSTM, tanh activation)
    ...
    def save_results(self, results, filename=None):
        """Save results with convergence plots to lstm_hpo_results directory"""
        # Create output directory
        output_dir = 'lstm_hpo_results_512'
        os.makedirs(output_dir, exist_ok=True)
        
        if filename is None:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            filename = os.path.join(output_dir, f"c4_lstm_fe_hpo_results_{timestamp}.json")
        else:
        ...
        final_results = {
            ...
            'model_type': 'Single_Layer_LSTM_with_13_features',
            'architecture': {
                'num_layers': 1,
                'lstm_activation': 'tanh',
                'epochs': 100,
...
def run_hyperparameter_optimization_from_files(sequence_length=144, n_calls=50,
    ...
    logger.info("="*80)
    logger.info("BAYESIAN HYPERPARAMETER OPTIMIZATION - LSTM with Feature Engineering")
    logger.info("="*80)
    
    try:
        csv_path = '1. Datasets/Train Sets/c4_train/train_data_c4.csv'
        ...
        logger.info(f"{'='*80}")
        logger.info(f"Model: Single-layer LSTM with 13-feature engineering")
...
if __name__ == "__main__":
    logger.info("="*80)
    logger.info("LSTM BAYESIAN OPTIMIZATION WITH FEATURE ENGINEERING")
    logger.info("="*80)

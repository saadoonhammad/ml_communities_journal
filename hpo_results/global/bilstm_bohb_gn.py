# ============================================================================
# BiLSTM AUTOENCODER - BAYESIAN HYPERBAND OPTIMIZATION (BOHB) - FUSED MODEL
# ============================================================================
"""
Bayesian Hyperband Optimization for Bidirectional LSTM Autoencoder using Optuna.
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'../../Logs/global_bilstm_bohb_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
...
OUTPUT_PREFIX = f'{CLUSTER}_global_bilstm_bohb_results'
...
TARGET_GPU = 0  #Change to 1 for RTX 
...
class FusedBiLSTMAutoencoder:
    ...
    def __init__(self, sequence_length: int, n_sensors: int,
        ...
        self.hidden_units = hidden_units          # units per direction
    ...
    def build_model(self):
        ...
        )(inp)   # output shape: (batch, 2 * hidden_units)
        ...
        )(decoded)   # output shape: (batch, sequence_length, 2 * hidden_units)
        
        # Time-distributed output layer
        out = TimeDistributed(Dense(self.n_sensors))(decoded)
...
def save_bohb_results(results: Dict[str, Any], output_dir: str = '../../HPO Results/Network_Global/global_bilstm_bohb_results'):
    ...
    json_results = {
        ...
        'model_architecture': 'BiLSTM_Autoencoder',   # added for clarity
        'cluster': CLUSTER,
...
if __name__ == "__main__":
    ...
    logger.info(f"Total BOHB optimization time: {overall_duration}")
    logger.info("="*80)
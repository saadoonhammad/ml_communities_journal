# ============================================================================
# PART 1: DATA PREPARATION AND MODEL TRAINING - MULTI-SENSOR LSTM AUTOENCODER
# ============================================================================
...
from tensorflow.keras.layers import Input, Dense, Dropout, Flatten, Reshape, LSTM, TimeDistributed, RepeatVector
...
OUTPUT_DIR = f'../Final Train/Network Global/lstm_trained_model/'
...
HIDDEN_UNITS = 67
ENCODING_DIM = 89
DROPOUT_RATE = 0.1252058709537588
LEARNING_RATE = 0.00019678039053164887
BATCH_SIZE = 64
EPOCHS = 150 
PATIENCE = 15 

# Additional BOHB parameter
PATIENCE_FACTOR = 0.14415714492516413
...
class FusedLSTMAutoencoder:
    """LSTM Autoencoder for multi-sensor time series with tanh activations"""
    ...
    def build_model(self):
        ...
        # Encoder: LSTM returns last state only
        x = LSTM(units=self.hidden_units, 
                 activation='tanh',
        ...
        # Output layer: reconstruct each timestep
...
if not os.path.exists(TRAIN_CSV_PATH):
    ...
    metadata = {
        'model_type': 'lstm_autoencoder_fused',
    ...
    print("Files created:")
    print("  - lstm_autoencoder.h5 (trained model)")
    ...
    print(f"  Architecture: LSTM({HIDDEN_UNITS}) → Dense({ENCODING_DIM}) → Repeat → LSTM({HIDDEN_UNITS}) → Output")

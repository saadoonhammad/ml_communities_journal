# ============================================================================
# BILSTM AUTOENCODER - BAYESIAN HYPERBAND OPTIMIZATION (BOHB)
# ============================================================================
"""
Bayesian Hyperband Optimization (BOHB) for BiLSTM Autoencoder using Optuna.
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'../Logs/bilstm_bohb_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
...
OUTPUT_PREFIX = f'{CLUSTER}_bilstm_bohb_results'
...
logger.info("="*80)
logger.info("BILSTM AUTOENCODER - BAYESIAN HYPERBAND OPTIMIZATION (BOHB)")
logger.info("="*80)
...
class BiLSTMAutoencoder:
    """Bidirectional Long Short-Term Memory (BiLSTM) Autoencoder for time series anomaly detection."""
    
    def __init__(self, sequence_length, n_features,
                 lstm_units=64, encoding_dim=32, 
    ...
    def build_model(self):
        ...
        encoded = Bidirectional(LSTM(self.lstm_units, activation='tanh', dropout=self.dropout_rate, 
        ...
        decoded = Bidirectional(LSTM(self.lstm_units, activation='tanh', dropout=self.dropout_rate, 
...
class BayesianHyperbandOptimizer:
    """
    Bayesian Hyperband Optimization (BOHB) for BiLSTM Autoencoder.
    ...
    def objective(self, trial: optuna.Trial) -> float:
        ...
        """
        # Define hyperparameter search space for BiLSTM
        params = {
            'lstm_units': trial.suggest_int('lstm_units', 64, 256, log=True),
    ...
    def run_optimization(self) -> Dict[str, Any]:
        ...
        self.study = optuna.create_study(
            ...
            sampler=sampler,
            pruner=pruner,
            study_name=f"bilstm_bohb_{CLUSTER}",
...
def save_bohb_results(results: Dict[str, Any], output_dir: str = '../HPO Results/Representative/bilstm_bohb_results'):
...
def plot_bohb_convergence(results: Dict[str, Any], filename: str):
    ...
    ax1.set_title('BOHB Optimization Convergence (BiLSTM)', fontsize=14, fontweight='bold')
...
def plot_parameter_importance(study: optuna.Study, filename: str):
    ...
    try:
        ...
        ax.set_title('Hyperparameter Importance (BiLSTM BOHB)', fontsize=14, fontweight='bold')
